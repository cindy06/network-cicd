...
podTemplate(label: 'k8s-pipeline', containers: [
    containerTemplate(name: 'docker', image: 'docker:1.12.6', command: 'cat', ttyEnabled: true),
    containerTemplate(name: 'node', image: 'node:latest', command: 'cat', ttyEnabled: true),
...
volumes:[
  ...
  node ('k8s-pipeline') {
    ...
    if (config.pipeline.debug) {
      ...
      println "Runing kubectl/helm connection tests"
      container('kubectl') {
    ...
    // WIP. There should be a better way.
    stage ('publish container') {
      container('docker') {
        ...
        withCredentials([[
                          ...
                          credentialsId   : config.container_repo.jenkins_creds_id,
    ...
    if (env.BRANCH_NAME =~ "PR-*" ) {
      stage ('deploy to k8s for PR testing') {
        container('helm') {
          ...
          //  Run helm tests if configured (integration tests)
          if (config.app.test) {
            pipeline.helmTest(
              name        : env.BRANCH_NAME.toLowerCase()
            )
    ...
    if (env.BRANCH_NAME == 'master') {
      stage ('deploy to k8s') {
        container('helm') {
          ...
          //  Run helm tests again (if configured) against prod deployment(integration tests)
          if (config.app.test) {
            pipeline.helmTest(
              name          : config.app.name
            )
